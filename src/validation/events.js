...
export const createEventSchema = Joi.object({
  title: Joi.string().min(5).max(16).required().messages({
  ...
  organizer: Joi.string().min(3).max(16).required().messages({
...
export const updateEventSchema = Joi.object({
    title: Joi.string().min(5).max(16).messages({
      'string.base': 'Title should be a string',
      'string.min': 'Title should have at least {#limit} characters',
      'string.max': 'Title should have at most {#limit}characters',
    }),
    ...
    organizer: Joi.string().min(3).max(16).messages({
      'string.base': 'Organizer: should be a string',
      'string.min': 'Organizer should have at least {#limit} characters',
      'string.max': 'Organizer should have at most {#limit} characters',
    }),
